var ExampleFaultTrees = (function () {
	...
	// 添加节点到指定父节点
	ExampleFaultTrees.prototype.addNodeToParent = function (pParentNode, pNewNode) {
		if (pParentNode && pParentNode.addChild) {
	...
	// 从父节点中移除子节点
	ExampleFaultTrees.prototype.removeNodeFromParent = function (pParentNode, pNodeToRemove) {
		if (pParentNode && pParentNode.mChildren) {
			var index = pParentNode.mChildren.indexOf(pNodeToRemove);
			if (index > -1) {
				pParentNode.mChildren.splice(index, 1);
				return true;
			}
		}
		return false;
	...
	// 查找包含指定节点的父节点
	ExampleFaultTrees.prototype.findParentNode = function (pRootNode, pTargetNode) {
		if (!pRootNode || !pRootNode.numChildren) {
			return null;
		}

		for (var i = 0; i < pRootNode.numChildren(); i++) {
			var child = pRootNode.getChild(i);
			if (child === pTargetNode) {
				return pRootNode;
			}

			var found = this.findParentNode(child, pTargetNode);
			if (found) {
				return found;
			}
		}
	...
	// 将故障树序列化为JSON
	ExampleFaultTrees.prototype.serializeFaultTree = function (pNode) {
		if (!pNode) return null;

		var nodeData = {
			id: pNode.getId(),
			text: pNode.getText(),
			type: this.getNodeType(pNode),
			children: []
		};

		if (pNode.numChildren && pNode.numChildren() > 0) {
			for (var i = 0; i < pNode.numChildren(); i++) {
				var child = pNode.getChild(i);
				nodeData.children.push(this.serializeFaultTree(child));
			}
		}
	...
	// 从JSON反序列化故障树
	ExampleFaultTrees.prototype.deserializeFaultTree = function (pData) {
		if (!pData) return null;

		var node = this.createNodeByType(pData.type);
		if (node) {
			// 不设置ID，让系统自动分配新的ID
			node.setText(pData.text);

			if (pData.children && pData.children.length > 0) {
				for (var i = 0; i < pData.children.length; i++) {
					var child = this.deserializeFaultTree(pData.children[i]);
					if (child) {
						node.addChild(child);
					}
				}
	...
	// 获取节点类型
	ExampleFaultTrees.prototype.getNodeType = function (pNode) {
		if (pNode instanceof BasicEvent) return 'BasicEvent';
		if (pNode instanceof AndGate) return 'AndGate';
		if (pNode instanceof OrGate) return 'OrGate';
		if (pNode instanceof TransferGate) return 'TransferGate';
		return 'Unknown';
	};

	// 根据类型创建节点
	ExampleFaultTrees.prototype.createNodeByType = function (pType) {
		switch (pType) {
			case 'BasicEvent':
